...
Note that this DE is not [[Separable Equations|separable]] nor [[Linear Differential Equations|linear]], so as of now (2/26/26), there are no tools under our belt that could possibly arrive us at an explicit solution. But let us factor $y^{2} - y^{4} = y^{2}(y+1)(y-1)$ which tells us $\frac{dy}{dx}=0$ when $y = -1, 0, 1$. Those are equilibrium solutions for our DE which divides the the interval of $y$ into 4 subintervals. 
